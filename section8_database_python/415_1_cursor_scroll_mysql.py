...
connection = pymysql.connect(
    ...
    cursorclass=pymysql.cursors.DictCursor  # To use unbuffered cursor it is necessary use SSDictCursor
)
...
with connection:
    ...
    with connection.cursor() as cursor:
        ...
        # EXECUTE COMMIT AFTER CREATE, UPDATE and DELETE IT IS NECESSARY
        connection.commit()
    ...
    # Create an UPDATE query to database
    with connection.cursor() as cursor:
        ...
        # cursor.fetchall() brings all data from database and iterate over each data
        # the cursor, after iterate for all data will be maintained at the last data iterated
        print('for 1')
        # It is possible to scroll the cursor two itens before
        cursor.scroll(-2)
        for row in cursor.fetchall():
            print(row)

        # It is possible to scroll the cursor for the absolute item index too
        print('for 2')
